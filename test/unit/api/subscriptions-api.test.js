...
const mockSubscriptions = require(path.join(TESTDIR, 'fixtures', 'subscriptions.json'));
...
test('getDeviceSubscriptions throws an Unauthenticated Error when credentials are wrong', async (
	t
) => {
...
test('getDeviceSubscriptions throws an error when subscription format is unsupported', async (
	t
) => {
	try {
...
test('getSubscriptionChanges returns all subscription updates since the specified timestamp', async (
	t
) => {
	try {
		let since = 12347;
